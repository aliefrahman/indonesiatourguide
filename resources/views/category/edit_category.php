<?php
// File: resources/views/category/edit_category.php
$pageTitle = "Edit Kategori Paket Wisata - IndoTour";
...
<div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="mb-8">
        <h1 class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Edit Kategori: <span class="text-teal-600"><?php echo htmlspecialchars($category['name_id']); ?></span></h1>
    </div>
        ...
        <form action="/categories/edit/<?php echo e($category['id']); ?>" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input id="slug" name="slug" type="text" required placeholder="beach" value="<?php echo htmlspecialchars($category['slug']); ?>"
                ...
                <div>
                    ...
                    <input id="icon" name="icon" type="text" placeholder="🏖️" value="<?php echo htmlspecialchars($category['icon'] ?? ''); ?>"
                        class="block w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 px-3.5 text-sm text-slate-800 font-medium focus:border-teal-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-teal-500 transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input id="name_id" name="name_id" type="text" required placeholder="Pantai & Selam" value="<?php echo htmlspecialchars($category['name_id']); ?>"
                        class="block w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 px-3.5 text-sm text-slate-800 font-medium focus:border-teal-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-teal-500 transition-all">
                </div>
                <div>
                    <label for="name_en" class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Nama Kategori (English)</label>
                    <input id="name_en" name="name_en" type="text" required placeholder="Beach & Diving" value="<?php echo htmlspecialchars($category['name_en']); ?>"
                        class="block w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 px-3.5 text-sm text-slate-800 font-medium focus:border-teal-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-teal-500 transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <textarea id="description_id" name="description_id" rows="4" placeholder="Deskripsi kategori..."
                        class="block w-full rounded-xl border border-slate-200 bg-slate-50 py-3 px-3.5 text-sm text-slate-800 focus:border-teal-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-teal-500 transition-all"><?php echo htmlspecialchars($category['description_id'] ?? ''); ?></textarea>
                </div>
                <div>
                    ...
                    <textarea id="description_en" name="description_en" rows="4" placeholder="Category description..."
                        class="block w-full rounded-xl border border-slate-200 bg-slate-50 py-3 px-3.5 text-sm text-slate-800 focus:border-teal-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-teal-500 transition-all"><?php echo htmlspecialchars($category['description_en'] ?? ''); ?></textarea>
                </div>
            </div>
