...
<div class="mx-auto max-w-3xl px-4 py-12">
    <div class="mb-8 flex items-center space-x-2">
        <a href="/tours/detail/<?php echo $package['id']; ?>" class="text-xs text-slate-500 hover:text-teal-400 font-bold transition-colors">
            <i data-lucide="chevron-left" class="mr-1"></i> <?php echo translate('Kembali ke Paket', 'Back to Package'); ?>
        </a>
    </div>
    ...
    <h1 class="text-3xl font-extrabold text-white tracking-tight mb-8 flex items-center space-x-3">
        ...
        <span><?php echo translate('Konfirmasi Pemesanan Wisata', 'Confirm Tour Booking'); ?></span>
    </h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="md:col-span-2 space-y-6">
            <div class="bg-slate-950/40 border border-slate-800/80 px-6 py-6 shadow-xl rounded-2xl backdrop-blur-md">
                ...
                <h3 class="text-sm font-bold text-white uppercase tracking-wider mb-5 pb-2 border-b border-slate-900"><?php echo translate('Lengkapi Data Keberangkatan', 'Departure Information'); ?></h3>
                
                <form action="/tours/checkout/<?php echo $package['id']; ?>" method="POST" class="space-y-5">
                    <?php echo \App\Core\Csrf::field(); ?>

                    <div>
                        <label for="travel_date" class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5"><?php echo translate('Tanggal Keberangkatan', 'Travel Date'); ?></label>
                    ...
                    <div>
                        <label for="total_participants" class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5"><?php echo translate('Jumlah Peserta (pax)', 'Total Participants'); ?></label>
                    ...
                    <div>
                        <label for="guide_id" class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5"><?php echo translate('Pilihan Pemandu Wisata', 'Select Tour Guide'); ?></label>
                        <select id="guide_id" name="guide_id" class="block w-full rounded-xl border border-slate-800 bg-slate-900/50 py-2.5 px-3.5 text-xs text-slate-200 focus:border-teal-500 focus:bg-slate-900 focus:outline-none focus:ring-1 focus:ring-teal-500 transition-all">
                            ...
                            <?php foreach ($guides as $guide): ?>
                                ...
                                <option value="<?php echo $guide['id']; ?>" <?php echo $isSel ? 'selected' : ''; ?>>
                    ...
                    <div class="pt-4 border-t border-slate-900 flex justify-between items-center text-sm font-bold">
                        <span class="text-slate-400"><?php echo translate('Estimasi Total Bayar:', 'Estimated Total Price:'); ?></span>
                        <span id="grand-total" class="text-lg text-teal-400"><?php echo format_rupiah($package['price']); ?></span>
                    </div>
                    ...
                    <div class="pt-4">
                        <button type="submit" 
                            ...
                            <i data-lucide="credit-card" class="mr-2 mt-0.5"></i> <?php echo translate('Proses Pembayaran Instan', 'Process Instant Payment'); ?>
                        </button>
                    </div>
                </form>
        ...
        <div class="md:col-span-1 space-y-6">
            <div class="bg-slate-950/20 border border-slate-800 p-5 rounded-2xl">
                <h4 class="text-xs font-bold text-slate-300 uppercase tracking-wider mb-4"><?php echo translate('Ringkasan Paket', 'Package Summary'); ?></h4>
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        ...
                        <span class="text-xs text-teal-400 font-bold block"><?php echo format_rupiah($package['price']); ?></span>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    const packagePrice = <?php echo (float)$package['price']; ?>;
